...
public class GameWindow {
    ...
    private boolean finishedScreenMade;
    ...
    public GameWindow(
        ...
        this.finishedScreenMade = false;
        pane = new Pane();
        scene = new Scene(pane, width, height);

        entityViews = new ArrayList<>();

        this.keyboardInputHandler = new KeyboardInputHandler(model, this);
    ...
    public void load(){
        model = memento.getState();
        keyboardInputHandler = new KeyboardInputHandler(model, this);
        scene.setOnKeyPressed(keyboardInputHandler::handlePressed);
        scene.setOnKeyReleased(keyboardInputHandler::handleReleased);
        backgroundDrawer.draw(model,pane);
        memento.setState(model.copy());
...
//        }
        ...
        for (Entity entity : entities) {
            //bad code I know but bro im tired
            if(entity instanceof DynamicEntityImpl && ((DynamicEntityImpl) entity).
                    checkDead()){
                continue;
            }
        ...
        ArrayList<Integer> levelScores = model.getLevelScores();

        //end screen
        if(model.isGameFinished() && finishedScreenMade == false){
            pane.getChildren().removeIf(Label.class::isInstance);
            ...
            for(int i = 0; i < levelScores.size(); i++){
                FinalScore += levelScores.get(i);
            }
            pane.getChildren().add(new Text(0, 20,"Total Score: " + FinalScore));
        }
        if(!model.isGameFinished()) {
            String output = "";
            for (int i = 0; i < levelScores.size(); i++) {
                output += "Score for Level " + (i + 1) + ": " + levelScores.get(i) + "\n";
            }
            output += "Score for Current Level: " + model.getCurrentLevelScore();
            Label text = new Label(output);
            text.setFont(Font.font("verdana", FontWeight.BOLD, FontPosture.REGULAR, 20));

            String levelColor = model.getLevelEnemeyColor();
            if(levelColor != null){
                text.setTextFill(Color.web(model.getLevelEnemeyColor()));
            }
            pane.getChildren().removeIf(Label.class::isInstance);
            pane.getChildren().add(text);
        }
